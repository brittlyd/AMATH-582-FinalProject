...
for iRot = 1:nRot
    ...
    Y(:,iRot) = reshape(data(iRot).Interp.Vmag_crop(~mask),[],1);
end
...
text(ind,energy(ind),strcat('  \leftarrow 90% of energy is captured, mode '...
    ,num2str(ind)),'VerticalAlignment','bottom','Fontsize',10);
ylabel('% of energy')
axis tight
yyaxis right
